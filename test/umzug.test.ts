...
describe('alternate migration inputs', () => {
  ...
  test('Error causes are propagated properly', async () => {
    ...
    await expect(umzug.down()).rejects.toThrowErrorMatchingInlineSnapshot(
      `[MigrationError: Migration m1 (down) failed: Original error: Some cryptic failure]`,
    )
  ...
  test('non-error throwables are wrapped helpfully', async () => {
    ...
    await expect(umzug.up()).rejects.toThrowErrorMatchingInlineSnapshot(
      `[MigrationError: Migration m2 (up) failed: Non-error value thrown. See info for full props: Some cryptic failure]`,
    )

    await expect(umzug.down()).rejects.toThrowErrorMatchingInlineSnapshot(
      `[MigrationError: Migration m1 (down) failed: Non-error value thrown. See info for full props: Some cryptic failure]`,
    )
  })
