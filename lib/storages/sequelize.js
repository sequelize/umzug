...
module.exports = redefine.Class({
  constructor: function (options) {
    ...
    if(!this.options.storageOptions.columnType) {
      // You may need to override this for utf8mb4 charsets
      var Sequelize;
      try {
        Sequelize = require(resolve.sync('sequelize', { basedir: process.cwd() }))
      } catch(e) {
        Sequelize = this.options.storageOptions.Sequelize;
      }
      if (!Sequelize) {
        throw new Error("sequelize dependency not found, Sequelize option is required");
      }
      this.options.storageOptions.columnType = Sequelize.STRING;
    }
